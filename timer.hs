...
makeWindowProc lpps = do
  ...
  varPresentTime <- newIORef (0,0,-1)
  varFontHeight <- newIORef fontHeight
  varSavedDC <- newIORef 0
  varAppState <- newIORef Idle
  return $ windowProc varTimeValue varPresentTime varFontHeight varSavedDC varAppState windowLocation lpps
...
windowProc varTimeValue varPresentTime varFontHeight varSavedDC varAppState windowLocation lpps hwnd wm wp lp
  | wm == wM_DESTROY = do
  ...
  | wm == wM_TIMER = do
      ...
      pre <- readIORef varPresentTime
      now <- getPresentTime 0
      if pre == now
        ...
        Idle ->
          ...
          case () of _
                       ...
                       | wp == vK_LEFT -> do -- See https://downloads.haskell.org/~ghc/7.8.1/docs/html/libraries/Win32-2.3.0.2/Graphics-Win32-Key.html
                           ff 60 -- I choose 'Left' key as increment because 'Left' key is nearer to 'Enter' key
                           return 0
                       | wp == vK_RIGHT -> do
                           ff (-60)
                           return 0
...
configFilePath = "countdown-timer-config"
